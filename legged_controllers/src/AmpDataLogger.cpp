...
AmpDataLogger::AmpDataLogger(const Config& config, ros::NodeHandle& nh) : config_(config) {
  if (config_.logToRos) {
    ...
    loggingCmdSub_ = nh.subscribe<std_msgs::Bool>("/amp/enable_logging", 1, &AmpDataLogger::loggingCmdCallback, this);
    gaitNameSub_ = nh.subscribe<std_msgs::String>("/amp/gait_name", 1, &AmpDataLogger::gaitNameCallback, this);
  }

  ROS_INFO("[AmpDataLogger] Initialized. Log dir: %s, Log to file: %d, Log to ROS: %d",
           config_.logDir.c_str(), config_.logToFile, config_.logToRos);
}
...
void AmpDataLogger::openFile() {
  ...
  std::stringstream ss;
  ss << config_.logDir << "/" << config_.logPrefix << "_" << std::setfill('0') << std::setw(4) << sequenceCount_ << ".csv";
...
void AmpDataLogger::log(double time, const vector_t& optimizedState, const vector_t& optimizedInput, const vector_t& wbcOutput,
                        const vector_t& measuredRbdState, size_t plannedMode, const CentroidalModelInfo& info) {
  if (!isLogging_) {
    return;
  }

  const double dt = 1.0 / config_.logFrequency;
  ...
  if (config_.logToRos) {
    publishRosData(time, optimizedState, optimizedInput, wbcOutput, plannedMode);
  }
  ...
  for (int i = 6; i < 12; ++i) {
    logFile_ << "," << (i < optimizedState.size() ? optimizedState(i) : 0.0);
  }
  ...
  const vector_t jointVels = centroidal_model::getJointVelocities(optimizedInput, info);
  for (int i = 0; i < jointVels.size(); ++i) {
    logFile_ << "," << jointVels(i);
  }

  const contact_flag_t contacts = modeNumber2StanceLeg(plannedMode);
  for (int i = 0; i < config_.numFeet; ++i) {
...
void AmpDataLogger::publishRosData(double time, const vector_t& optimizedState, const vector_t& optimizedInput,
                                   const vector_t& wbcOutput, size_t plannedMode) {
