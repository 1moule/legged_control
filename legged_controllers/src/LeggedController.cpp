...
bool LeggedController::init(hardware_interface::RobotHW *robot_hw, ros::NodeHandle &controller_nh) {
  ...
  eeKinematicsPtr_ =
      std::make_shared<PinocchioEndEffectorKinematics>(leggedInterface_->getPinocchioInterface(), pinocchioMapping,
  ...
  selfCollisionVisualization_.reset(new LeggedSelfCollisionVisualization(leggedInterface_->getPinocchioInterface(),
                                                                         leggedInterface_->getGeometryInterface(),
                                                                         pinocchioMapping,
                                                                         nh));
  ...
  std::vector<std::string> wheel_joint_names{"LF_foot_joint", "LH_foot_joint",
                                             "RF_foot_joint", "RH_foot_joint"};
  for (const auto &joint_name : wheel_joint_names) {
    wheelJointHandles_.push_back(hybridJointInterface->getHandle(joint_name));
  }
  ...
  // Sub
  auto fsmCmdCallback = [this](const std_msgs::BoolConstPtr &msg) {
    stateChange_ = true;
  };
  fsmCmdSub_ = controller_nh.subscribe<std_msgs::Bool>("/cmd_fsm", 1, fsmCmdCallback);
...
void LeggedController::starting(const ros::Time &time) {
  ...
  TargetTrajectories
      target_trajectories
      ({currentObservation_.time}, {leggedInterface_->getInitialState()}, {currentObservation_.input});
...
void LeggedController::update(const ros::Time &time, const ros::Duration &period) {
  // FSM
  switch (robotMode_) {
    case RobotMode::STAND_UP:standUp(time, period);
      break;
    case RobotMode::NORMAL:normal(time, period);
      break;
  }
}
...
void LeggedController::normal(const ros::Time &time, const ros::Duration &period) {
  // State Estimate
  ...
  for (auto &wheel : wheelJointHandles_)
    wheel.setCommand(0, 0, 0, 10, 0);
...
void LeggedController::standUp(const ros::Time &time, const ros::Duration &period) {
  hybridJointHandles_[0].setCommand(0, 0, 300, 30, 0);
  hybridJointHandles_[3].setCommand(0, 0, 500, 50, 0);
  hybridJointHandles_[6].setCommand(0, 0, 300, 30, 0);
  hybridJointHandles_[9].setCommand(0, 0, 500, 50, 0);

  hybridJointHandles_[1].setCommand(0.8, 0, 200, 30, 0);
  hybridJointHandles_[4].setCommand(0.8, 0, 200, 30, 0);
  hybridJointHandles_[7].setCommand(0.8, 0, 200, 30, 0);
  hybridJointHandles_[10].setCommand(0.8, 0, 200, 30, 0);

  hybridJointHandles_[2].setCommand(-1.63, 0, 250, 30, 0);
  hybridJointHandles_[5].setCommand(-1.63, 0, 500, 30, 0);
  hybridJointHandles_[8].setCommand(-1.63, 0, 250, 30, 0);
  hybridJointHandles_[11].setCommand(-1.63, 0, 500, 30, 0);

  for (auto &wheel : wheelJointHandles_)
    wheel.setCommand(0, 0, 0, 10, 0);

  if (stateChange_) {
//  if (currentObservation_.state(8) > 0.4) {
    robotMode_ = RobotMode::NORMAL;
    mpcRunning_ = true;
  }
}
...
void LeggedController::setupMpc() {
  ...
  auto gaitReceiverPtr =
      std::make_shared<GaitReceiver>(nh,
